...
if(isset($_POST['submit_password']) && !empty($_POST['username']) && !empty($_POST['password']))
{
  $user=$_POST['username'];
  $pass=$_POST['password'];
  $query1="UPDATE `secretaire` SET `password`='$pass' WHERE login = '$user'";
  $select1=mysqli_query($link,$query1);
  if($select1){
      header("Location: connexionSecretaire.php?newpwd=passwordupdated");
      exit();
  }
  else{
      header("Location: resetPasswordS.php?echec=هناك مشكلة ، حاول مرة أخرى");
      exit();
  }
}
    ...
    <section>
               ...
               <p class="login-text" style="font-size:1.3rem; font-weight: 680;">تعيين كلمة المرور</p>
                   ...
                   <div class="input-group" style="display:flex; flex-direction: row;">
                    ...
                    <input type="password" name='password' placeholder="new Password" id="password" required="required">
     ...
     header("Location: resetPasswordS.php?echec3=المعلومات غير صحيحة");
      exit();
  } 
}
?>