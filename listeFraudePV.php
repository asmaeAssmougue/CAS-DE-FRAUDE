<?php
 ...
 if(isset($_POST['submit'])){
    if(empty($_POST['anneeUnv'])||empty($_POST['session'])||empty($_POST['date'])){
        ...
        header("Location: listeFraudePV.php?error=المرجو ملىء  جميع الخانات&loginS=$loginS");
        exit(); 
    }else{
      ...
      switch ($session) {
                ...
                default:
                    header("Location: listeFraudePV.php?error1=المرجو الاختيار من الائحة&loginS=$loginS");
                    exit();
            }
      ...
      $_SESSION['date']=$date;
      header("Location: listeAvecPV.php?succes=1&loginS=$loginS");
      exit();
