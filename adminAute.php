...
if(empty($email)){
    header("Location: adminAute.php?error=الرجاء إدخال اسم المستخدم");
    exit();
}else if(empty($pass)){
    header("Location: adminAute.php?error1=من فضلك أدخل رقمك السري");
    exit();
}else{ 
    ...
    if(mysqli_num_rows($reslt) === 1){
        ...
        if($row['login'] == $email && $row['password'] == $pass){
            ...
            header("Location: admin.php?succes= مرحبًا بك ");
            exit();
        }
        }
    else{
           header("Location: adminAute.php?error2=كلمة السر أو اسم المستخدم غير صحيحة");
           exit();
